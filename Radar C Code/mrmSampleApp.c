...
#define DEFAULT_RADIO_IP			"192.168.0.220"
...
static void socketServerListen()
{
    ...
    if ((new_sockfd = accept(sockfd, (struct sockaddr *)&serv_addr, (socklen_t*)&serv_addr)) < 0) {
    ...
    switch (info->msg.scanInfo.msgType)
    {
		...
		case MRM_FULL_SCAN_INFO:
			...
			if (fp) {
				...
				send(new_sockfd, &(info->msg.scanInfo.numSamplesTotal), sizeof(info->msg.scanInfo.numSamplesTotal), 0);
				
				for (i = 0; i < info->msg.scanInfo.numSamplesTotal; i++)
				{
					fprintf(fp, ", %d", info->scan[i]);
					send(new_sockfd, &(info->scan[i]), sizeof(info->scan[i]), 0);
				}
				fprintf(fp, "\n");
			}
            free(info->scan);
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		int valread = read(new_sockfd, buffer, 1);
		printf("Received from client: %s\n", buffer);
		if (strcmp(buffer, "0") == 0)
			break;
		else 
			userScanCount = atoi(buffer);
